...
export default function AccountPage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-[#f7f8f8]">身份</h1>
      ...
      <div className="border border-[rgba(255,255,255,0.08)] rounded-xl overflow-hidden mb-6 bg-[rgba(255,255,255,0.02)]">
        ...
        <div className="px-6 pb-6 -mt-8">
          <div className="flex items-end gap-4 mb-4">
            ...
            <div className="pb-1">
              <h2 className="text-xl font-bold text-[#f7f8f8]">{user?.name ?? "加载中..."}</h2>
      ...
      {workspaces.length > 0 && (
        <div className="mb-6">
          <h2 className="text-lg font-semibold mb-3 text-[#f7f8f8]">工作区 ({workspaces.length})</h2>
      ...
      {/* My Agents */}
      <h2 className="text-lg font-semibold mb-3 text-[#f7f8f8]">我的 Agent ({agents.length})</h2>
      <div className="grid grid-cols-2 gap-3">
        {agents.map(a => (
          <div key={a.id} className="p-4 border border-[rgba(255,255,255,0.08)] rounded-lg bg-[rgba(255,255,255,0.02)]">
            ...
            <div className="text-xs text-[#8a8f98] mb-2">{a.description?.slice(0, 80) ?? "暂无描述"}</div>
            {a.capabilities?.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                ...
                {a.capabilities.length > 4 && (
                  <span className="text-xs text-[#8a8f98]">+{a.capabilities.length - 4} 更多</span>
                )}
              </div>
            )}
            {a.workspace_id && (
        ...
        {agents.length === 0 && <div className="col-span-2 text-[#8a8f98]">暂无 Agent</div>}
      </div>
    </div>
  )
}
