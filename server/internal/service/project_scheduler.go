...
func (s *ProjectSchedulerService) triggerProject(ctx context.Context, p db.Project) error {
	...
	// Find the workflow linked to this plan by scanning running/pending workflows.
	// We use ListWorkflowsByStatus to find candidates and match by plan_id.
	wf, err := s.findWorkflowByPlan(ctx, util.UUIDToString(plan.ID))
	if err != nil {
		slog.Warn("project scheduler: no workflow found for plan, skipping execution",
			"project_id", projectID, "plan_id", util.UUIDToString(plan.ID), "error", err)
		return nil
	}

	workflowID := util.UUIDToString(wf.ID)

	// Kick off execution via the scheduler.
	if err := s.scheduler.ScheduleWorkflow(ctx, workflowID, runID); err != nil {
...
// findWorkflowByPlan searches for the most recent workflow associated with the given plan ID.
func (s *ProjectSchedulerService) findWorkflowByPlan(ctx context.Context, planID string) (db.Workflow, error) {
	// Check workflows in various states that could be associated with the plan.
	for _, status := range []string{"pending", "ready", "completed", "failed"} {
		workflows, err := s.queries.ListWorkflowsByStatus(ctx, status)
		if err != nil {
			continue
		}
		for _, wf := range workflows {
			if util.UUIDToString(wf.PlanID) == planID {
				return wf, nil
			}
		}
	}

	return db.Workflow{}, fmt.Errorf("no workflow found for plan_id=%s", planID)
}
